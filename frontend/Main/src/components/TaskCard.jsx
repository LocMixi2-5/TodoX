...
const TaskCard = ({ task, index, handleTaskChanged }) => {
    ...
    const updateTask = async () => {
        try {
            ...
            const old_name = task.title;

            setIsEditting(false);
            await api.put(`/tasks/${task._id}`, {
                title: updateTaskTitle
            });
            toast.success(`Nhiệm vụ ${old_name} đã đổi tên thành ${updateTaskTitle}.`);
            handleTaskChanged();
        } catch (error) {
            console.log("Lỗi xảy ra khi đổi tên task.",error);
            toast.error("Lỗi xảy ra khi đổi tên nhiệm vụ.");
    ...
    const toggleTaskCompleteButton = async () => {
        try {
            ...
            console.log("Lỗi xảy ra khi đổi trạng thái task.",error);
    ...
    return (
        <Card className={cn(
            ...
            style={{animationDeloy: `${index * 50}ms`}}
        >
