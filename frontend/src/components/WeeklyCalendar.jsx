    ...
    <div className="w-full max-h-[600px] overflow-auto bg-white/70 border border-white/80 rounded-2xl relative shadow-inner scrollbar-thin scrollbar-thumb-teal-300 scrollbar-track-transparent">
      <div className="flex sticky top-0 bg-white/95 backdrop-blur-md z-20 border-b border-slate-200/80 shadow-sm">
        ...
        {days.map((day) => (
          <div
            key={day}
            className="flex-1 text-center py-3 text-xs font-extrabold text-slate-700 border-r border-slate-200/60 last:border-0 uppercase tracking-wide"
          >
            {day}
          </div>
        ))}
        ...
        <div className="flex-1 flex relative">
          {days.map((day, dayIndex) => {
            ...
            const dayBlocks = schedules.filter(
              (s) => s.dayOfWeek === dbDayOfWeek,
            );

            return (
              <div
                key={dayIndex}
                className="flex-1 border-r border-slate-200/60 relative group"
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col h-full justify-start text-[10px] leading-tight text-white font-bold tracking-tight py-0.5">
                        {/* Dòng 1: Tên môn */}
                        <div
                          className="text-[11px] truncate pr-4 drop-shadow-sm mb-0.5"
                        ...
                        <div className="opacity-95 truncate drop-shadow-sm mb-0.5">
                          Thời gian: {formatDecimalTime(block.startHour)}-
                          {formatDecimalTime(block.endHour)}
                        </div>

                        {/* Dòng 3: Giảng viên */}
                        {block.instructor && (
                          <div className="opacity-90 truncate drop-shadow-sm mb-0.5">
                            Người dạy: {block.instructor}
                          </div>
                        )}
